	...
	public function generate($_FPOST)
	{
		...
		for ($i = 0; $i < count($column); $i++) {
			...
			if ($column[$i] == trim($primaryKey)) {
				...
				$htmlInputs .= '<div class="col-md-12">
								...
								<select style="width:100%" id="' . $inputName . '" name="' . $inputName . '" class="form-select" ' . $inputRequired . '>
								</select>
				...
				success: function(result) {
					...
					$("#dataForm' . $uControlerName . ' #' . $inputName . '").select2({
						data: result,
					})
					$("#dataForm' . $uControlerName . ' #' . $inputName . '").val(null);
					$("#dataForm' . $uControlerName . ' #' . $inputName . '").trigger("change");
					$(document).on("select2:open", () => {
						document.querySelector(".select2-search__field").focus();
					});
					$("#dataForm' . $uControlerName . ' #' . $inputName . '").select2({
						dropdownParent: $("#dataModal' . $uControlerName . '")
					});
      ...
      ';
				...
				success: function(result) {
					...
					$("#dataForm' . $uControlerName . ' #' . $inputName . '").val(response.' . $inputName . ');
					$("#dataForm' . $uControlerName . ' #' . $inputName . '").trigger("change");
					$(document).on("select2:open", () => {
						document.querySelector(".select2-search__field").focus();
					});
      ...
      ';
			} elseif ($iType[$i] == '8' && $crudShow == '1') {


				$htmlInputs .= '<div class="col-md-12 mt-2">
							<div class="form-group">
								<div class="icheck-primary d-inline">
									...
									<input required="" name="' . $inputName . '" id="' . $inputName . '" type="checkbox">

								</div>
								<label style="margin-left:10px" for="' . $inputName . '">' . $inputlabel . ': ' . $htmlInputRequired . '</label>
							</div>
						</div>' . "\n";
				...
				$CheckBoxEdit .= '			
			if(response.' . $inputName . ' == 1){
				$("#dataForm' . $uControlerName . ' #' . $inputName . '").prop("checked", true);
			}';
			} elseif ($iType[$i] == '9' && $crudShow == '1') {


				$htmlInputs .= '
				
				<fieldset class="row mb-3">
                                            <legend class="col-form-label col-sm-2 pt-0">' . $inputlabel . ': ' . $htmlInputRequired . '</legend>
                                            <div class="col-sm-10">
                                                <div class="form-check"> <input class="form-check-input" type="radio" name="' . $inputName . '" id="' . $inputName . '1" value="1"> <label class="form-check-label" for="' . $inputName . '1">
                                                        Sim
                                                    </label> </div>
                                                <div class="form-check"> <input class="form-check-input" type="radio" name="' . $inputName . '" id="' . $inputName . '2" value="2"> <label class="form-check-label" for="' . $inputName . '2">
                                                        Não
                                                    </label> </div>
                                            </div>
                                        </fieldset>' . "\n";
				...
				$RadioEdit .= '			
			if(response.' . $inputName . ' == 1){
				$("#' . $inputName . '1").prop("checked", true);
			}
			if(response.' . $inputName . ' == 2){
				$("#' . $inputName . '2").prop("checked", true);
			}';
			...
			if ($iType[$i] == '8' && $crudShow == '1') {


				$ciFields .= '
				
				if($this->request->getPost(\'' . $inputName . '\')=="on"){
					$fields[\'' . $column[$i] . '\'] = 1;
				}else{
					$fields[\'' . $column[$i] . '\'] = 0;
				}' . "\n";
			...
			if ($column[$i] == $primaryKey) {
				$ciValidationAdd .= '            \'' . $column[$i] . '\' => [\'label\' => \'' . $inputlabel . '\', \'rules\' => \'' . 'permit_empty' . '\'],' . "\n";
			} else {
				$ciValidationAdd .= '            \'' . $column[$i] . '\' => [\'label\' => \'' . $inputlabel . '\', \'rules\' => \'' . $ciValidationRequired . $ciValidationType . $ciValidationMinlength . $ciValidationMaxlength . $ciValidationUnique . '\'],' . "\n";

			}


			$ciValidationEdit .= '            \'' . $column[$i] . '\' => [\'label\' => \'' . $inputlabel . '\', \'rules\' => \'' . $ciValidationRequired . $ciValidationType . $ciValidationMinlength . $ciValidationMaxlength . $ciValidationUnique . '\'],' . "\n";
			if ($dtShow[$i] == '1') {
			...
			if ($crudShow == '1' and $iType[$i] != '8' and $iType[$i] !== '6') $htmlEditFields .= '			$("#dataForm' . $uControlerName . ' #' . $inputName . '").val(response.' . $column[$i] . ');' . "\n";
		}
		...
		$find   = ['@@@table@@@', '@@@primaryKey@@@', '@@@allowedFields@@@', '@@@controlerName@@@', '@@@uControlerName@@@', '@@@modelName@@@', '@@@uModelName@@@', '@@@crudTitle@@@', '@@@htmlInputs@@@', '@@@ciFields@@@', '@@@ciValidationAdd@@@', '@@@ciValidationEdit@@@', '@@@ciDataTable@@@', '@@@htmlDataTable@@@', '@@@htmlEditFields@@@', '@@@ciSelect@@@', '@@@SelectAdd@@@', '@@@SelectEdit@@@', '@@@CheckBoxEdit@@@', '@@@RadioEdit@@@'];
		$replace   = [$table, $primaryKey, $allowedFields, $controlerName, $uControlerName, $modelName, $uModelName, $crudTitle, $htmlInputs, $ciFields, $ciValidationAdd,$ciValidationEdit, $ciDataTable, $htmlDataTable, $htmlEditFields, $ciSelect, $SelectAdd, $SelectEdit, $CheckBoxEdit, $RadioEdit];
