								...
								<div class="row">
									<div class="col-md-4">
										<div class="form-group text-left">
											...
											<select id="pessoaFiltro" name="codColaborador" class="custom-select" required>
												<option value=""></option>
											</select>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="confirmacaoDesativacaoForm" class="pl-3 pr-3">
					...
					<input type="hidden" id="codColaboradorConfirmacaoDesativacao" name="codColaborador" class="form-control" placeholder="Código" maxlength="11" required>
					...
					<button type="button" class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Fechar" data-bs-dismiss="modal">Cancelar</button>
			</div>
		</div>
		</form>
	</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="add-form-pessoa" class="pl-3 pr-3">
					...
					<input type="hidden" id="codColaboradorAdd" name="codColaborador" class="form-control" placeholder="Código" maxlength="11" required>
								...
								<div class="tab-content" id="custom-tabs-one-tabContent">
									<div class="tab-pane fade show active" id="dados-pessoais" role="tabpanel" aria-labelledby="dados-pessoais-tab">
										<form id="edit-form-pessoa" class="pl-3 pr-3">
											...
											<input type="hidden" id="codColaboradorFormAdmin" name="codColaborador" class="form-control" placeholder="Código" maxlength="11" required>
											...
											<div class="form-group text-center">
												<div class="btn-group mt-4">
													...
													<button type="button" class="btn btn-xs btn-success" id="btnTrocasenhaCodColaborador" id="btnTrocasenhaCodColaborador">Troca Senha</button>
													<button type="button" class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Fechar" data-bs-dismiss="modal">Fechar</button>
												</div>
											</div>
										</form>
...
<script>
	...
	function edit(codColaborador) {




		document.getElementById("btnTrocasenhaCodColaborador").onclick =
			function() {
				trocasenha(codColaborador)
			}
		...
		$.ajax({
			...
			success: function(response) {
				...
				codColaboradorTmp = codColaborador;
				$('#data_tablefuncoesAtribuidas').DataTable({
					...
					"responsive": true,
					"ajax": {
						"url": '<?php echo base_url('funcoesAtribuidas/pegaFuncoesPessoa/') ?>' + "/" + codColaborador,
						"type": "POST",
						"dataType": "json",
						async: "true",
						data: {
				...
				$("#edit-form-pessoa #codColaboradorFormAdmin").val(response.codColaborador);
				...
				$("#edit-form-pessoa #codBenFormAdmin").val(response.codBen);
				...
				$.validator.setDefaults({
					...
					submitHandler: function(form) {
						...
						if ($('#fotoPerfilFormAdmin')[0].files[0] !== 'undefined') {
							...
							formData.append('codColaborador', codColaboradorTmp);
							formData.append('csrf_principal', $("#csrf_principal").val());
							$.ajax({
								...
								success: function(resultFoto) {
									if (resultFoto.success == true) {
										...
										if (resultFoto.meuCodColaborador == codColaboradorTmp) {
	...
	function confirmacaoDesativacao(codColaborador, nomeExibicao) {
		$("#confirmacaoDesativacaoForm")[0].reset();
		$('#confirmacaoDesativacaoModal').modal('show');
		$(".form-control").removeClass('is-invalid').removeClass('is-valid');

		$("#confirmacaoDesativacaoForm #codColaboradorConfirmacaoDesativacao").val(codColaborador);
	...
	function desativarPessoaAgora() {
		...
		codColaborador = $("#confirmacaoDesativacaoForm #codColaboradorConfirmacaoDesativacao").val();
	...
	function reativarPessoa(codColaborador) {
		Swal.fire({
...
<script>
	function addFuncoesAtribuidasModel() {
		...
		$("#add-formFuncoesAtribuidasModal #codColaborador").val(codColaboradorTmp);
		// submit the add from 
		$.validator.setDefaults({
			highlight: function(element) {
				$(element).addClass('is-invalid').removeClass('is-valid');
	...
	function editfuncoesAtribuidas(codColaboradorFuncao) {
		$.ajax({
			url: '<?php echo base_url('funcoesAtribuidas/getOne') ?>',
			type: 'post',
			data: {
				codColaboradorFuncao: codColaboradorFuncao,
				csrf_principal: $("#csrf_principal").val(),
			...
			success: function(response) {
				...
				$("#edit-form-funcoes-atribuidas #codColaboradorFuncao").val(response.codColaboradorFuncao);
				$("#edit-form-funcoes-atribuidas #codColaborador").val(response.codColaborador)
	...
	function removefuncoesAtribuidas(codColaboradorFuncao) {
		Swal.fire({
			title: 'Você tem certeza que deseja remover?',
			text: "Você não poderá reverter após a confirmação",
			icon: 'warning',
